...
void solve(){
    int n,m;cin>>n>>m;
    int mini[n][m],maxi[n][m],v[n][m];
    for(int i=0;i<n;i++) for(int j=0;j<m;j++) cin>>v[i][j];
    mini[0][0] = maxi[0][0] = v[0][0];
    for(int i=1;i<n;i++){
        mini[i][0] = maxi[i][0] = v[i][0] + mini[i-1][0];
    }
    for(int j=1;j<m;j++){
        mini[0][j] = maxi[0][j] = v[0][j] + mini[0][j-1];
    }
    for(int i=1;i<n;i++){
        for(int j=1;j<m;j++){
            mini[i][j] = v[i][j] + min(mini[i-1][j],mini[i][j-1]);
            maxi[i][j] = v[i][j] + max(maxi[i-1][j],maxi[i][j-1]);
        }
    }
    if((n+m)%2==0 || mini[n-1][m-1]%2 || maxi[n-1][m-1]<0 || mini[n-1][m-1]>0) PNO;
    else PYES;
    return;
}
