...
void solve() {
    ...
    for(int i=0;i<n-1;i++){
        if(l.size()>2*k) break;
        if(a[i]==a[i+1]){
            l.pb(a[i]);l.pb(a[i+1]);a[i]=a[i+1]=0;
        }
    }
    for(int i=0;i<n-1;i++){
        if(r.size()>2*k) break;
        if(b[i]==b[i+1]){
            r.pb(b[i]);r.pb(b[i+1]);b[i]=b[i+1]=0;
        }
    }
    sort(rall(a));sort(rall(b));
    for(int i=0;i<n && l.size()<2*k;i++){
        l.pb(a[i]);r.pb(b[i]);
    }
    for(int i=0;i<2*k;i++) cout<<l[i]<<" ";cout<<endl;
    for(int i=0;i<2*k;i++) cout<<r[i]<<" ";cout<<endl;
    return;
}
