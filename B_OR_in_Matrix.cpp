...
void solve(){
    ...
    bool flag=true,countr=0,countc=0;
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            if(v[i][j] && row[i]!=m &&  col[j]!=n){
                // cout<<row[i]<<" "<<col[j]<<endl;
                flag=false;break;
            }
            if(col[j]==n && !countc) countc  = 1;
            if(row[i]==m &&  !countr)  countr  = 1;
        }
        if(!flag) break;
    }
    // cout<<countr<<" "<<countc<<endl;
    if(!flag || (countc^countr)){
        PNO;return;
    }
    PYES;
    for(int i=0;i<n;i++){
