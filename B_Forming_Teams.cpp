...
void solve(){
    ...
    function<void (int,int)> dfs =[&](int st,int prev){
        vis[st] = 1;
        count++;
        for(auto u:adj[st]){
            if(u==prev) continue;
            if(vis[u]){
                cycle = true;           // the best part here is that // Each student has at most two archenemies.
            }else dfs(u,st);            // if in a connected component if there is a cycle // the whole component is cycle
        }
    };
    int ans=0;
    for(int i=1;i<=n;i++){
