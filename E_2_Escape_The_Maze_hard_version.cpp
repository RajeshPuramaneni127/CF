...
set<int> s;
void dfs(vector<vector<int>> &adj,vector<int> &par,int st,int prev){
    if(par[prev]!=1){
        par[st] = par[prev];
    }
    for(auto u:adj[st]){
    ...
    if(st!=1 && adj[st].size()==1) s.insert(par[st]);
}
void solve(){
    //Vlad starts the game in the room 1 and wins if he reaches a room other than 1, into which exactly one corridor leads.
    // which means vlad has to go from 1 to any of the leaf node!!
    // what we need check?
    // the distance from the root to any of the leaf < distance from any friend to any leaf
    int n,k;cin>>n>>k;
    vector<int> a(k);
    ...
    queue<int> q;
    vector<int> dist(n+1,n+1),par(n+1);
    for(int i=0;i<k;i++){
        dist[a[i]] = 0;
        q.push(a[i]);
        par[a[i]] = a[i];
