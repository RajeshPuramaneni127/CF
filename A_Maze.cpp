...
vector<string> adj(501,"");
vector<vector<bool>> visited(501,vb(501,false));
vi dx = {0,0,-1,1};
vi dy = {-1,1,0,0};
int n,m,k;
void dfs(int x,int y){
    if(visited[x][y] || k==0) return;
    visited[x][y] = true;
    bool flag = false;
    for(int i=0;i<4;i++){
        int newx = x+ dx[i];
        int newy = y+ dy[i];
        if(newx>=0 && newx<n && newy>=0 && newy<m && adj[newx][newy]=='.' && !visited[newx][newy]){
            dfs(newx,newy);
            if(adj[newx][newy]=='.'){ // this point was highlight it will cover ever empty cell
                flag = true;
            }
        }
    }
