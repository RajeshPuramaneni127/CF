...
int powm(int x,int y,int M){if(x==0 && y==0) return (int)1;if(x==0) return (int)0;if(y==0) return (int)1;if(y==1) return x;int k = powm(x,y/2,M);if(y%2) return x*k*k;else return k*k;}
...
int  dp[20][4][2];// indx,non_zero elements, smaller?
int rec(const string& s,int ind,int non_zero,bool small){
    if(ind == s.length()) return 1;
    if(non_zero==3) return 1;
    if(non_zero>=4) return 0;
    // if the present index value is 0
    if(dp[ind][non_zero][small]!=-1) return dp[ind][non_zero][small];
    int res = rec(s,ind+1,non_zero,small? small : (s[ind]!='0'));
    if(small){
        res+=9*rec(s,ind+1,non_zero+1,1);
    }else{
        int k = s[ind]-'0'-1;
        if(k>0) res+= k*rec(s,ind+1,non_zero+1,1);
        if(s[ind]!='0') res+=rec(s,ind+1,non_zero+1,0);
    }
    return dp[ind][non_zero][small] = res;
}
void solve(){
    int l,r;cin>>l>>r;
    //cout<<rec(to_string(r),0,0,0)-rec(to_string(l-1),0,0,0)<<endl;
    memset(dp,-1,sizeof(dp));
    int ans = rec(to_string(r),0,0,0);
    memset(dp,-1,sizeof(dp));
    ans-= rec(to_string(l-1),0,0,0);
    cout<<ans<<endl;
    return; 
}
signed main(){
