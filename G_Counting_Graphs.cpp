...
void solve(){
    int n,s;cin>>n>>s;
    vector<int> deg(n,0);
    vector<vector<int>> edg(n,vector<int> (3));
    // int x,y,w;
    for(int i=0;i<n;i++){
    ...
    for(int i=0;i<n;i++){
        if(x<=0) break;
        if(!vis[edg[i][1]]){
            ans+=(((s-edg[i][0])*((powm(2,x-deg[edg[i][1]]-1,modd)-1+modd))%modd))%modd;x--;
            vis[edg[i][1]] = true;
        }
        if(!vis[edg[i][2]]){
            ans+=(((s-edg[i][0])*((powm(2,x-deg[edg[i][2]]-1,modd)-1+modd))%modd))%modd;x--;
            vis[edg[i][2]] = true;
        }
    }
    cout<<ans<<endl;
