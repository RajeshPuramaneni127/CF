...
void dfs(vector<int> adj[], int p, int c, priority_queue<long long> &v, long long *dpth, long long *sub){
    sub[c] = 1;
    for (auto u: adj[c]) if (u != p) {
        dpth[u] = dpth[c] + 1;
        dfs(adj, c, u, v, dpth, sub);
        sub[c] += sub[u];
    }
    v.push(dpth[c] - sub[c] + 1);
}
void solve(){
    int n, k;
    cin >> n >> k;
