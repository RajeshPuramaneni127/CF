...
int n,w,f;
int find(vector<vector<int>>& dp,vector<int>& st,int curr){
    int tot = accumulate(all(st),0);
    int ans = 1e9;
    int l = 0, r = 1e9;
    while (l <= r) {
        int mid = l + (r - l) / 2;
...
void solve() {
    ...
    vector<vector<int>> dp(2, vector<int>(1e6 + 4, 0));
    dp[0][0] = 1;
    dp[0][st[0]] = 1;
    int curr = 1;
    int i = 1;
    while (i < n) {
        ...
        while (j < 1e6 + 2) {
            ...
            if (j >= st[i])
                dp[curr][j] = max(dp[curr][j], dp[1 - curr][j - st[i]]);
            j++;
        }
        curr = 1 - curr;
        i++;
