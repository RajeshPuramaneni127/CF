...
// A great person said 
// If you can't implement it, simply you don't understand it well enough....
void solve(){
    int n,l,r,ql,qr;
    cin>>n>>l>>r>>ql>>qr;
    ...
    int ans=-1;
    for(int i=0;i<=n;i++){
        int curr= (pre[i]-pre[0])*l+(pre[n]-pre[i])*r;
        if(i<n-i) curr += (abs(n-2*i-1))*qr;
        else if(i>n-i) curr+=(abs(2*i-n-1))*ql;
        if(ans==-1) ans =curr;
        ans = min(ans,curr);
    }
    cout<<ans<<endl;
