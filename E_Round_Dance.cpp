...
bool flag= 0;
void dfs(vector<set<int>> &adj,vector<bool> &vis,vector<int> &degree,int st){
    if(degree[st]==1) flag= true;
    vis[st] = true;
    for(auto u:adj[st]){
        if(!vis[u]) dfs(adj,vis,degree,u);
    }
    return;
}
void solve(){
    ...
    for(int i=1;i<=n;i++){
        if(!vis[i]){
            flag= 0;
            dfs(adj,vis,degree,i);
            if(flag) b++;
            else c++;
        }
    }
    cout<< c+min(b,(int)1)<<" "<<c+b<<endl;
    return;
}
signed main(){
