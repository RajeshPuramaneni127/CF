...
int  n,m;
vector<int> adj[100001],dp(100001),ans(100001,1);
...
void dfs2(int st,int prev){
    ans[st] = (((ans[prev]-1+m)%m)*powm(dp[st],m-2,m)+1)%m; // you have to crack this logic 
    ans[st] = (ans[st]*((dp[st]-1+m)%m))%m; // that's it 
    for(auto u:adj[st]){
        if(u==prev) continue;
        dfs(u,st);
    }
