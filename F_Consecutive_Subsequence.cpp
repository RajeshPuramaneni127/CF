...
void solve(){
    int n;cin>>n;
    vi dp(n+1,0),par(n+1,0);
    int ind = 0,maxi = INT_MIN,x;
    map<int,int> mpp;                // you might get errors when you use unordered_map for higher inputs
                                     // donot know the reason!!!
    for(int i=1;i<=n;i++){
        cin>>x;
            if(dp[mpp[x-1]]+1>=dp[i]){
                dp[i] = dp[mpp[x-1]]+1;
                mpp[x] = i;
            }
        if(dp[i]>maxi){
