...
void solve(){
    ...
    for(int i=1;i<=m;i++){
        cin>>a>>b;
        a--;
        mpp[v[a].first].push_back({v[a].second,i});
        v[a] = {b,i};
    }
    for(int i=0;i<n;i++){
        mpp[v[i].first].push_back({v[i].second,m+1});
    }
    int ans =0;
    for(auto& [k,val]:mpp){
        int present_time = 0;
        for(auto&d:val){
            present_time += (d.second-d.first);
        }
        int abs_time = m+1-present_time;
        ans+= (present_time*abs_time + (present_time*(present_time-1))/2);
        //cout<<present_time<<" "<<abs_time<<endl;
    }
    cout<<ans<<endl;
    return;
}
