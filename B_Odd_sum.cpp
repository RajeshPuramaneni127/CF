...
void solve(){
    ...
    vi a(n);
    for(auto& u:a) cin>>u;
    vi odd(n,INT_MIN),even(n,INT_MIN);
    if(a[0]%2) odd[0] = a[0];
    else even[0] = a[0];
    for(int i=1;i<n;i++){
        if(a[i]%2){
            if(even[i-1]!=INT_MIN){
                odd[i] = max({odd[i-1],a[i],a[i]+even[i-1]});
            }else{
                odd[i] = max(odd[i-1],a[i]);
            }
            if(odd[i-1]!=INT_MIN){
            ...
            if(even[i-1]!=INT_MIN){
                even[i] = max({even[i-1],a[i],a[i]+even[i-1]});
            }else{
                even[i] = max(even[i-1],a[i]);
            }
            if(odd[i-1]!=INT_MIN){
                odd[i] = max(odd[i-1],odd[i-1]+a[i]);
            }else{
                odd[i] = odd[i-1];
            }
        }
