...
void solve(){
    ...
    int  ans = INT_MAX;
    if(t_zero>s_zero || t_one>s_one) ans=0;
    if(t_zero!=0)  ans=  min(ans,s_zero/t_zero);
    if(t_one!=0) ans  =  min(ans,s_one/t_one);
    for(int i =1;i<=ans;i++){
        cout<<k;
    }
    s_zero-=ans*(t_zero);
    s_one-=ans*(t_one);
    for(int i=0;i<k.size();i++){
        if(k[i]=='1' && s_one-->0) cout<<"1";
        else if(k[i]=='0' && s_zero-->0) cout<<"0";
    }
    while(s_zero>0){
        cout<<"0";s_zero--;
    }
