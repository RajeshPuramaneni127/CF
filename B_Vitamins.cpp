...
void solve(){
    ...
    int ans = min(mpp["A"] + mpp["B"] + mpp["C"],mpp["ABC"]);
    ans = min(ans,mpp["A"] + mpp["BC"]);
    ans = min(ans,mpp["B"]+mpp["AC"]);
    ans = min(ans,mpp["C"]+mpp["AB"]);
    ans = min(ans,mpp["BC"] + mpp["AC"]);
    ans = min(ans,mpp["AB"] + mpp["BC"]);           // LOL!!!!
    ans = min(ans,mpp["AB"] + mpp["AC"]);
    if(ans==INT_MAX){
        cout<<-1<<endl;
    }else cout<<ans<<endl;
    return;
