...
vector<int> t[maxn];
int freetime(int arr,int i){
    int x = lower_bound(all(t[i]),arr)-t[i].begin();
    if(x>=t[i].size() || t[i][x]>arr) return 0;
    int l = x,r = t[i].size()-1;
    int ans = t[i].size();
    while(l<=r){
        int mid = l + (r-l)/2;
        if(t[i][mid]-t[i][x] != mid-x){
            ans = mid;
            r = mid-1;
        }else{
            l = mid+1;
...
void solve(){
    ...
    while(!pq.empty()){
        ...
        vis[b] = true;
        for(auto [u,d]:adj[b]){
            int x = dist[b] + d + freetime(dist[b]+d,u);
            if(u==n-1) x = dist[b]+d;
            if(x<dist[u]){
                dist[u] = x;
                pq.push({-dist[u],u});
