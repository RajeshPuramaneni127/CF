...
//     // vi f(n+1,0);
//     for(int k=0;k<=n;k++){
//         int maxi = 0,sum=0,l=0;
//         for(int r=0;r<n;r++){
//             sum+=a[r];
//                 while(l<=r && sum+min(k,r-l+1)*x<=0){
//                     sum-=a[l];l++;
//                 }
//                 maxi = max(maxi,sum+min(k,r-l+1)*x);
//         }
//         cout<<maxi<<" ";
//     }cout<<endl;
//     return;
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        for(int j=i;j<n;j++){
            sum+=a[j];
            sub[j-i+1] = max(sub[j-i+1],sum);
        }
    }
    for(int k=0;k<=n;k++){
        int maxi = 0;
        for(int i=0;i<=n;i++){
            maxi = max(maxi,sub[i]+min(i,k)*x);
        }
        cout<<maxi<<" ";
    }
    cout<<endl;
