...
void solve(){/*
    ...
    for(int i=1;i<=n;i++){
        for(int j=i;j<=n;j++){
            count.push_back(sums[j]-sums[i-1]);
        }
    }
    sort(all(count));
    for(int i=0;i<count.size();i++){
        if(a%count[i]==0){
            ans+=upper_bound(count.begin()+i+1,count.end(),a/count[i])-lower_bound(count.begin()+i+1,count.end(),a/count[i]);
        }
    }
    cout<<ans<<endl;
    */
   ll x; cin >> x;
	...
	rep(i,0,n) {
		...
		rep(j,i,n) {
			sum += s[j]-'0';
			if (sum == 0 || x%sum == 0) cnt[sum]++;
		}
	}
	
	if (x == 0) {
		ll res = n * (n+1) * cnt[0] - cnt[0]*cnt[0];
		cout << res << endl;
		return;
	}
	...
	for (ll d = 1; d*d <= x; d++) if (x%d == 0) {
		ll cur = cnt[d] * cnt[x/d];
		res += 2*cur;
		if (d == x/d) res -= cnt[d]*cnt[d];
	}
	cout << res << endl;
    return;
}
