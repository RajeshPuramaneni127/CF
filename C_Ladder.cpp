...
void solve(){
    ...
    if(!binary_search(right.begin(),right.end(),n-1)){
        right.push_back(n-1);
    }
    int l,r;
    while(m--){                // My mistake was in this loop only
        cin>>l>>r;
        l--;r--;
        if(l==r){
            PYes;continue;
        }
        int x = lower_bound(all(left),l)-left.begin();
        if(left[x]!=l){
            x--;
        }
        int y = lower_bound(all(right),r)-right.begin();
        if(x==y){
            PYes;
        }else{
            PNo;
