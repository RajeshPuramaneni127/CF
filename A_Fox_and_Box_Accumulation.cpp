...
void solve(){
    // piles are arranged from smallest to largest 
    // if there are k piles till the index i
    // so minimum no of boxes in any of these piles will be i/k;
    // because what I am doing is that equally in all these piles so that maximum no of boxes from all these piles 
    // should be minimized
    // for i = 7 piles =3 arrangement is like
	//  0 1 2
	//  3 4 5
	//  6 7   -- so ith block must have to at least i/k to support above blocks
	//  i.e 7th block must support at least two blocks above it (1,4)
    int n;cin>>n;
    vi a(n);
    ...
    for(int i=0;i<n;i++){
        while(a[i]<i/k){
            k++;          // extra pile was added // so all the boxes arranged in prev k-1 piles
                          // will now be arranged equally in all these k piles 
        }
    }
    cout<<k<<endl;
    return; 
